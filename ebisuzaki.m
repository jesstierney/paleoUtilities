function [F,rSim] = ebisuzaki(x,y,sig,nsim);
...
[rr,pr] = corrcoef(x,y,'rows','pairwise'); rReal = rr(2,1);
...
[rs,ps] = corrcoef(standardize(X(:,i)),standardize(Y(:,i))); rSim(i) = rs(2,1);
...
disp('---')
disp(['Observed correlation coefficent: ', num2str(rReal)])
